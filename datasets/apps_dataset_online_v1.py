...
class APPSBaseDataset(torch.utils.data.Dataset):
    def __init__(self, dataroot, problem_dirs, model, max_tokens, sample_mode, 
        ...
        if self.model in ['codet5-base', 'codet5-large', 'codet5-large-ntp-py']:
            self.tokenizer = transformers.RobertaTokenizer.from_pretrained('models/codet5-base')
    ...
    def sample_rl_task(self, item):
        if not self.fine_grained_feedback:
            input_ids, labels, gt_error, error_logit, baseline_error, gen_pass_ratio, baseline_pass_ratio = item
        else:
            input_ids, labels, gt_error, error_logit, baseline_error, gen_pass_ratio, baseline_pass_ratio, error_line, reward_type = item

            fine_grained_error_mask = find_position(self.tokenizer, labels, error_line, reward_type).astype(float)

            # mask = find_position(self.tokenizer, labels, error_line, reward_type)
            # mask_tokens = labels[mask]
            # decode_error_line = self.tokenizer.decode(mask_tokens)
            #
            # assert error_line in decode_error_line, f'error_line:\n{error_line}\ndecode_error_line:\n{decode_error_line}\n'
    ...
    def debug(self):
        ...
        for idx in range(len(self)):
            ...
            q_str, s_str, gt_code_str, answer_type = sample[0]
...
class APPSRepairDataset(torch.utils.data.Dataset):
    def __init__(self, dataroot, problem_dirs, model, max_tokens, max_src_tokens):
        super(APPSRepairDataset, self).__init__()
        self.dataroot = dataroot
        self.problem_dirs = problem_dirs
    ...
    def initialize(self):
        ...
        for problem_name in tqdm(self.problem_dirs):
            ...
            if (not os.path.isfile(question_fname)) or (not os.path.isfile(sols_fname))\
                    or (not os.path.isfile(input_output_fname)) or (not os.path.isfile(gen_sols_fname)):
                skipped_problems.append(problem_name)
                continue
            ...
            sols_str_list = json.load(open(sols_fname, 'r'))
            gen_sols = json.load(open(gen_sols_fname, 'r'))

            samples, infos = self.load_repair_sample(sols_str_list, answer_type, starter_code, question_str,
                                              gen_sols)
            self.update_error_stat(infos)
            all_samples += samples
            samples_info += infos
        ...
        assert len(all_samples) == len(samples_info)
        print("Error subtype distribution: {}".format(sorted(Counter(self.all_error_subtypes).items())))

        self.samples = all_samples
        self.samples_info = samples_info

    def load_repair_sample(self, gt_sols, answer_type, starter_code, question_str, gen_sols):
        samples = []
        infos = []

        for gen_sol in gen_sols:
            gen_code = gen_sol['code']
            result = gen_sol['result']
            error_type = gen_sol['error_type']

            if result is True:
                continue

            for gt_sol in gt_sols:
                sample = [question_str, starter_code, gt_sol, answer_type, gen_code]
                samples.append(sample)
                infos.append((result, error_type))
    ...
    def __getitem__(self, item):
        raw_samples = self.pack_samples(item)
        inputs = self.sample_task(raw_samples)
    ...
    def pack_samples(self, idx):
        ...
        curr_question, curr_start, curr_gt_code, curr_answer_type, curr_gen_code = self.samples[idx]
        curr_result, curr_error_subtype = self.samples_info[idx]

        curr_question = curr_question[:150000]
        curr_start = curr_start[:150000]
        curr_gt_code = curr_gt_code[:150000]
        curr_gen_code = curr_gen_code[:150000]

        curr_samples.append((curr_question, curr_start, curr_gt_code, curr_answer_type,
                             curr_gen_code, curr_result, curr_error_subtype))
        return curr_samples
    ...
    def sample_task(self, raw_samples):
        ...
        q_str, s_str, gt_code_str, answer_type, gen_code_str, result, error_subtype = sample
        ...
        answer_token_ids = self.tokenizer.encode(gt_code_str, verbose=False)
        label_ids.extend(answer_token_ids)

        input_ids_max_len = self.max_src_tokens if self.model in ['codet5-base', 'codet5-large',
                                                                  'codet5-large-ntp-py'] else self.max_tokens
        if len(input_ids) < input_ids_max_len:
            new_input_ids = [self.tokenizer.eos_token_id] * input_ids_max_len
            new_input_ids[:len(input_ids)] = input_ids
            input_ids = new_input_ids
        ...
        if self.model not in ['codet5-base', 'codet5-large', 'codet5-large-ntp-py'] and len(input_ids) != len(
            label_ids): pdb.set_trace()
