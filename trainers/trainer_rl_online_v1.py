...
from transformers.deepspeed import deepspeed_init, is_deepspeed_zero3_enabled #, deepspeed_reinit
    ...
    def train(
        ...
        **kwargs,
        ...
        #for total_step in range(200000):
            ...
            # update data
            if (total_step + 1) % (update_freq * args.gradient_accumulation_steps) == 0:
                print(f"total_step: {total_step}  update_freq: {update_freq}")
                gen_mean_reward, gen_mean_reward_w_baseline, mean_reward, mean_reward_w_baseline, \
                num_new_files, num_new_samples = self.train_dataset.update_sample(update_root)

                logs = dict()
                logs['gen_mean_reward'] = round(gen_mean_reward, 4)
                logs['gen_mean_reward_w_baseline'] = round(gen_mean_reward_w_baseline, 4)
                logs['mean_reward'] = round(mean_reward, 4)
                logs['mean_reward_w_baseline'] = round(mean_reward_w_baseline, 4)
                logs['num_new_files'] = num_new_files
                logs['num_new_samples'] = num_new_samples
                self.log(logs)
            ...
            if (total_step + 1) % steps_in_epoch == 0:  # epoch end
