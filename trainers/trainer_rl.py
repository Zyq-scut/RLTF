...
class Trainer_RL:
    ...
    def compute_loss(self, model, inputs, step, return_outputs=False):
        ...
        if self.tuning_mode in ['critic']: 
            ...
            # print(f"input_ids: {inputs['input_ids'].shape}  labels: {inputs['labels'].shape}  "
            #       f"error_types: {inputs['error_types'].shape}  error_preds  {error_preds.shape}")
            # codegen
            # input_ids: torch.Size([2, 1312])  labels: torch.Size([2, 1312])
            # error_types: torch.Size([2, 1])  error_preds  torch.Size([2])

            # codet5
            # input_ids: torch.Size([8, 800])  labels: torch.Size([8, 512])
            # error_types: torch.Size([8, 1])  error_preds  torch.Size([8])
