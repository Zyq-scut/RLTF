# coding=utf-8
# Copyright 2023 Tencent authors, 2022 Salesforce authors, The EleutherAI, and HuggingFace Teams. All rights reserved.
#
...
_CHECKPOINT_FOR_DOC = "models/codegen-2B-mono"
_CONFIG_FOR_DOC = "CodeGenConfig"
_TOKENIZER_FOR_DOC = "GPT2Tokenizer"


CODEGEN_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "models/codegen-350M-nl",
    "models/codegen-350M-multi",
    "models/codegen-350M-mono",
    "models/codegen-2B-nl",
    "models/codegen-2B-multi",
    "models/codegen-2B-mono",
    "models/codegen-6B-nl",
    "models/codegen-6B-multi",
    "models/codegen-6B-mono",
    "models/codegen-16B-nl",
    "models/codegen-16B-multi",
    "models/codegen-16B-mono",
...
class CodeGenForCausalLM(CodeGenPreTrainedModel):
    ...
    def forward(
        ...
        if rewards is not None:
            # Shift so that tokens < n predict n
            shift_rl_logits = lm_logits[..., :-1, :].contiguous()
            shift_labels = labels[..., 1:].contiguous()
            shift_rewards = rewards[..., 1:].contiguous()

            log_prob_fct = nn.LogSoftmax(dim=-1)
            probs = log_prob_fct(shift_rl_logits)[shift_labels != -100].unsqueeze(0)
            rl_seqs = shift_labels[shift_labels != -100].unsqueeze(0)
            rl_probs = torch.gather(probs, 2, rl_seqs[:, :, None]).squeeze(-1)
            rl_rewards = shift_rewards[shift_labels != -100].unsqueeze(0)
