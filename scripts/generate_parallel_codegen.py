...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='exps/codegen_copy/checkpoint-29000',
    ...
    parser.add_argument('--num_seqs_per_iter', type=int, default=10,
                        help='number of possible minibatch to generate programs per iteration, depending on GPU memory')
    parser.add_argument('--num_seqs', type=int, default=100, help='number of total generated programs per test sample')
    ...
    for k in range(K):
        ...
        output_path = os.path.join(args.output_path, 'codegen_' + model_name + f'_{args.num_seqs:04d}_none_bs1x4_lr2e-05-100-1')
        ...
        for i in range(gpu_num):
            ...
            render_cmd = 'CUDA_VISIBLE_DEVICES=%d python generate_codegen.py ' \
                         '--model_path %s ' \
                         '--tokenizer_path %s ' \
                         '--test_path %s ' \
                         '--output_path %s ' \
