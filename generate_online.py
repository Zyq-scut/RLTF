...
import argparse
import torch
from generate import generate_critic_inputs, generate_prompt
...
critic_model = T5ForConditionalGeneration.from_pretrained(args.critic_path, tuning_mode='critic')
...
def generate_one_problem(model, problem_id):
    ...
    #test_example_path = args.test_example_path + '{}'.format(str(problem_id).zfill(4))


    if not os.path.exists(starter_path):
        starter_path = None

    input_text = generate_prompt(args, test_case_path, prompt_path, None, tokenizer, starter_path)

    with torch.no_grad():
        input_ids = torch.LongTensor(tokenizer.encode(input_text, verbose=False, max_length=args.source_len)).unsqueeze(0).cuda()
...
def generate_all():
    ...
    while True:
        ...
        if len(models) == 0:
            ...
            latest_model = T5ForConditionalGeneration.from_pretrained(latest_model)
        else:
            models.sort(key=os.path.getctime, reverse=True)
            print(models)
            for path in models:
                try:
                    latest_model = T5ForConditionalGeneration.from_pretrained(path)
        ...
        while time.time() - start < args.interval:
            ...
            try:
                time1 = time.time()
                generate_one_problem(latest_model, problem_id)
